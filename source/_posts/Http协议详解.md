...
![](https://m.360buyimg.com/img/jfs/t1/33909/21/16019/11258/630da1d3E3e84bb36/0e6c5dabe0624929.jpg)
...
`http://www.xx.com:8080/news/index.asp?boardID=5&ID=24618&page=1#name`

从上面的URL可以看出，一个完整的URL包括以下几部分：
- 协议部分：该URL的协议部分为“http：”，这代表网页使用的是HTTP协议。在Internet中可以使用多种协议，如HTTP，FTP等等本例中使用的是HTTP协议。在"HTTP"后面的“//”为分隔符

- 域名部分：该URL的域名部分为“www.xx.com”。一个URL中，也可以使用IP地址作为域名使用
...
URL是Internet上用来描述信息资源的字符串，主要用在各种WWW客户程序和服务器程序上，特别是著名的Mosaic(一个早期的浏览器)。
...
http请求由三部分组成，分别是：请求行、消息报头(请求头部)、请求正文

![](https://m.360buyimg.com/img/jfs/t1/219025/5/21626/5794/630da4c2E4371cfdf/1ede4d563be57c54.png)
...
- POST方法要求被请求服务器接受附在请求后面的数据，常用于提交表单。POST请求例子，使用Charles抓取的request如下：
...
### Http指纹识别技术
Http指纹识别的原理大致上也是相同的：
记录不同服务器对Http协议执行中的微小差别进行识别.Http指纹识别比TCP/IP堆栈指纹识别复杂许 多,理由是定制Http服务器的配置文件、增加插件或组件使得更改Http的响应信息变的很容易,这样使得识别变的困难；然而定制TCP/IP堆栈的行为 需要对核心层进行修改,所以就容易识别.
